...
let
  ...
  rime-ice-pkgs = pkgs.callPackage ./rime-ice.nix { };
...
in
{
      ...
      default =
        {
          ...
          linux = "${librime'}/bin/rime_deployer --build ${cfg.dir}/ ${librime'}/share/rime-data/* ${cfg.dir}/build && fcitx5-remote -r";
          # linux = "${librime'}/bin/rime_deployer && fcitx5-remote -r";
  ...
  config = mkIf cfg.enable {
    home.file.${cfg.dir} = {
      source = pkgs.callPackage ./rime-ice.nix { };
      recursive = true;
      onChange = cfg.deploy;
    };
